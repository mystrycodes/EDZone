...
export const purchaseCourse = async (req,res)=>{
    try {
        ...
        // Data stored to the database
        const newPurchase = await Purchase.create(purchaseData)

        // Initilizing the Stripe gateway
    ...
    try {
        ...
        if(progressData){
            if(progressData.lectureCompleted.includes(lectureId)){
                return res.json({success:true, message:"Lecture Already Watched"})
            }
        ...
        res.json({success:true,message:"Updated Progress"})
    } catch (error) {
        res.json({success:false,message:error.message})
    }
}
